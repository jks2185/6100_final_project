...
monk_data <- read.csv("data/monks-1.csv")

head(monk_data)

drop_cols <- c("data_id")
monk_data <- monk_data[, !(names(monk_data) %in% drop_cols)]

unique(monk_data$class)
...
for(i in 1:20){
  train <- sample(1:nrow(monk_data), 0.7*nrow(monk_data))
  validate <- setdiff(1:nrow(monk_data), train)
  
  train_set <- monk_data[train,]
  valid_set <- monk_data[validate,]
