...
autism_data <- read.csv("data/autism-adult-data.csv")
head(autism_data)


autism_data <- dummy_cols(autism_data, select_columns = c("ethnicity", "country", "age_desc", "relation"))

drop_cols <- c("ethnicity", "country", "age_desc", "relation", "autistm")
autism_data <- autism_data[, !(names(autism_data) %in% drop_cols)]


autism_data$gender <- ifelse(autism_data$gender == "m",1,0)
autism_data$jundice <-ifelse(autism_data$jundice == "yes",1,0)
#autism_data$autistm <-ifelse(autism_data$autistm == "yes",1,0)
autism_data$used_app_before <-ifelse(autism_data$used_app_before == "yes",1,0)
autism_data$class <-ifelse(autism_data$class == "YES",1,0)

unique(autism_data$class)


autism_data <- autism_data[!(autism_data$age == "383" | autism_data$age == "?"),]
autism_data$age <- as.integer(autism_data$age)
class(autism_data$age)

names(autism_data) <- sub(" ", ".", names(autism_data))
names(autism_data) <- gsub(x = names(autism_data), pattern = "\\-", replacement = "_")
names(autism_data) <- gsub(x = names(autism_data), pattern = "\\?", replacement = "U")
names(autism_data) <- gsub(x = names(autism_data), pattern = "\\(", replacement = "_")
names(autism_data) <- gsub(x = names(autism_data), pattern = "\\)", replacement = "_")
names(autism_data) <- gsub(x = names(autism_data), pattern = "\\&", replacement = "_")
names(autism_data) <- gsub(x = names(autism_data), pattern = "\\'", replacement = "")
...
for(i in 1:20){
  train <- sample(1:nrow(autism_data), 0.7*nrow(autism_data))
  validate <- setdiff(1:nrow(autism_data), train)
  
  train_set <- autism_data[train,]
  valid_set <- autism_data[validate,]
...
dt_timing <- system.time({
  dt_model <- train(as.factor(class) ~ ., data = train_set, method = "rpart", trControl = ctrl)
